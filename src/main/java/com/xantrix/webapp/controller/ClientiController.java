...
import com.xantrix.webapp.service.CouponsService;
...
public class ClientiController 
{
	...
	{
		Clienti cliente;
		List<Clienti> recordset = new ArrayList<Clienti>();
		...
		{
			cliente = clientiService.SelCliente(pSearchTerm);
			
			List<Coupons> coupons = couponsService.SelByCodFidelity(cliente.getCodFidelity());
			
			if (coupons != null)
			{
				DoubleSummaryStatistics CouponsValueStat = 
						coupons
						.stream()
						.collect(Collectors.summarizingDouble(p -> p.getValore()));
				
				cliente.setValCoupons(CouponsValueStat.getSum());
			}
			
			recordset.add(cliente);
	...
	public String GestUpdClienti(
			...
			@Valid @ModelAttribute("Utente") Utenti utente, 
